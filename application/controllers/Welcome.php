...
class Welcome extends CI_Controller {
	...
	function getcommission(){
	     $date=date("Y-m-d");
            $lastDate = date('Y-m-d',(strtotime ( '-15 day' , strtotime ( $date) ) ));
             $startDate = date('Y-m-d',(strtotime("-1 month",strtotime($date))));
             $backyear = date("Y",strtotime($startDate));
             $backMonth = date("m",strtotime($startDate));
             $backDate = $backyear."-".$backMonth."-01";
        }
}
